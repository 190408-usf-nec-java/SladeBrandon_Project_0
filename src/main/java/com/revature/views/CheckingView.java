...
public class CheckingView implements View {
	...
	public View menuCommands() {
		...
		System.out.println("  Thank you for choosing to create a checking account.");
		...
		case 3: {
				...
				if (userSet.contains(username_Entry)) {
					try (Connection connection = ConnectionUtil.getConnection()) {
						...
						String sql = "insert into bank_account_table (account_name, balance) values ('Checking', 0.00) RETURNING account_number";
						PreparedStatement ps = connection.prepareStatement(sql);
						ResultSet rs = ps.executeQuery();
						while (rs.next()) {
						...
						System.out.println("  The user you entered doesn't exist please try again.");
